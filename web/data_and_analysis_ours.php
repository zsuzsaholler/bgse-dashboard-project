...
?>
	...
	<p>We use the <b><a href="http://grouplens.org/datasets/hetrec-2011/" target="_blank">LAST.FM dataset</a></b> which contains information on approximately 18,000 artists, 1,900 users, 12,000 end-user tags, 13,000 connections among users and 93,000 user-artist connections with listen count information. The dataset used for the analysis was cleaned to make it easily accessible and manageable. Note, that the dataset lacks time dimension for the listen count information, which makes the analysis challenging.</p>
	
<p>Our main variable of interest is the success of an artist measured by the overall number of listens of the given artist, recorded in the Last.fm system. Let's see the top 20, most popular artists, based on their number of listens.</p>
...
<h2>Centrality</h2>
<p>Do the opinions of more central users matter more? Is it more likely you will become successful if more influential users listen to your music? Intuitively, the more central a user is, the more influential he should be. We therefore expect the success of a particular artist to be directly linked with the centrality of his fans. We combine the information on social connections of users and their listening history to define the artist level centrality measure used in the regression analysis.</p>
...
<p>Our final measure used in the regression analysis is the mean level of centrality of users listening to a given artist. The chart below shows the average centrality of listeners for each of the top 20 successful artists.</p>
...
<h2>Tag Count</h2>
<p>Do the opinions of more active users have a greater effect? Is it more likely you will become successful if your fans are more active in the system? Number of tags assigned by a user is another aspect of the user's social behavior and is a measure of user activity. Similar to central users, we expect that more active users will be more influential and thus the artists they listen to to be more successful.</p>
<p>To define our level of activity measure used in the regression analysis, first we count the number of tags created by each user (on all artists) and then aggregated this to artist level by taking the average number or tags created by an artist's listeners. The chart below shows the mean of the number of tags assigned by the listeners of the top 20 successful artists.</p>
...
<p>Can the opinions and feelings of users expressed in tags help to predict the success of an artist? The first step in analyzing whether users opinions and feelings could be indicative of an artist's popularity is to identify the sentiments embodied in each of the tags assigned by the users. To achieve this we used an <abbr title="Minqing Hu and Bing Liu. Mining and Summarizing Customer Reviews. Proceedings of the ACM SIGKDD International Conference on Knowledge Discovery and Data Mining (KDD-2004), Aug 22-25, 2004, Seattle, Washington, USA">exhaustive list</abbr> of words  classified from very positive to very negative and trained a <b><a href="https://en.wikipedia.org/wiki/Naive_Bayes_classifier" target="_blank">Naive Bayesian Classifier</a></b> to determine the overall sentiment of tags. After classifying all of the tags, we defined our final input variable used in the regression analysis; the ratio of positive to total number of tags assigned to a given artist.</p>

<p>Initially, we classified by hand a small subset of tags as positive, neutral or negative based on their overall meaning. Next, we counted the number of positive/neutral/negative individual words in these tags using the list of pre-classified words. We then trained the Naive Bayes classifier on a random subset of classified tags and tested the predictions on the remaining set of classified tags. The table below shows the predictive success of the classifier on our training and test data. It can be seen that the automatized classification is not perfect, but it serves our purposes since it finds a fair amount of the positive tags and more importantly it produce only a relatively small number of false positive results. Finally, the whole set of tags were classified using the Naive Bayes classifier.
</p>
...
<p> Before we turn to the regression analysis let's see a few examples! Please select an artist to see what the network of his or her users looks like! Check out the tags related to that artist as well! Blue colour indicates tags classified as positive. Let's see if we can identify any patterns or differences between artists. </p>
...
<p>In the final step of the analysis we run an artist level regression in order to explore the relationship between the characteristics of the audience of an artist and his/her success. Our regression is a simple linear regression with non-linear transformations to account for curvature and skewness of the independent variables. We also control for the amount of time an artist has been present in the database by looking at the date they were first tagged.</p>
<p>We run a regression on the total sample of artists, as well as a subset which is made up of artists who only appeared in the database in the last 16 months and is therefore meant to represent new and up and coming artists. The table below shows the results of these two regressions.</p>
<p>We get statistically significant coefficient estimates for all our indepedent variables apart from those relating to tags in the new and up and coming subset which we put down to lack of tag data. As expected, both listener average tag count and the ratio of positive to total tags, positively affect artist listen count. We also find that listener mean centrality has a positive affect on artist success, however our regression fits poorly for artists with very high mean user centrality. </p>	
<?php
